...
async function main() {
    try {
        ...
        server.tool("create_order", "Create a new regular order", {
        ...
        server.tool("batch_create_orders", "Create multiple orders in batch", {
        ...
        server.tool("edit_order", "Edit an existing order", {
        ...
        server.tool("cancel_order", "Cancel an order by order ID", {
            order_id: z.string().describe("Order ID to cancel"),
        }, async ({ order_id }) => {
            const result = await Orders.cancelOrder(order_id);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        server.tool("cancel_order_by_client_id", "Cancel an order by client order ID", {
        ...
        server.tool("cancel_all_pending_orders", "Cancel all pending orders", {
        ...
        server.tool("cancel_all_after", "Cancel all orders after a timeout", {
            timeout: z.number().describe("Timeout in milliseconds"),
        }, async (params) => {
            const result = await Orders.cancelAllAfter(params);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        server.tool("get_orders", "Get orders with filtering options", {
        ...
        server.tool("get_order_by_id", "Get order by order ID", {
            order_id: z.string().describe("Order ID to retrieve"),
        }, async ({ order_id }) => {
            const result = await Orders.getOrderById(order_id);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        server.tool("get_order_by_client_id", "Get order by client order ID", {
        ...
        server.tool("create_algo_order", "Create an algorithmic order (stop loss, take profit, etc.)", {
            symbol: z.string().describe("Trading symbol"),
        ...
        server.tool("edit_algo_order", "Edit an existing algo order", {
        ...
        server.tool("get_algo_orders", "Get algo orders with filtering options", {
            symbol: z.string().optional().describe("Filter by symbol"),
        ...
        server.tool("get_algo_order_by_id", "Get algo order by order ID", {
            order_id: z.string().describe("Algo order ID to retrieve"),
        }, async ({ order_id }) => {
            const result = await Orders.getAlgoOrderById(order_id);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        });
        server.tool("get_algo_order_by_client_id", "Get algo order by client order ID", {
            client_order_id: z.string().describe("Client order ID to retrieve"),
        }, async ({ client_order_id }) => {
        ...
        server.tool("get_position_history", "Get position history", {
